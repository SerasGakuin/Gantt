...
const changeMaterialsClass = {
  ...
  start() {
    ...
    console.log(`${changeMaterialsClass._LOGTAG}: シートは、${thisYearNum}/${thisMonthNum}のデータを格納しているものと認識しました。`);
    spIOManager.monthlySheet_createBackUp();//月間管理のバックアップをとる

    console.log(`${changeMaterialsClass._LOGTAG}: 新しい教材計画の元データの作成を開始します。`);
    //「ID（年月のprefixなし）」「教科名」「参考書名」「月間目標」「週ごとの計画時間(五列)」
    // 「週ごとの実績(五列)」「単位当たり処理量」が入った二次元配列を取得。
    const updatedDataMatrix = this._dataGetter.getUpdatedDataMatrix(spIOManager, todayObj);

    //このlogは多分重めなので運用時はコメントアウトしてください。
    //console.log("新しいデータの配列（これを基にデータを貼り付けする）\n"+updatedDataMatrix.map(row => row.join(', ')).join('\n'));

    console.log(`${changeMaterialsClass._LOGTAG}: 新しい教材計画の元データ作成完了しました。\n貼りつけ開始します。`);
  ...
  */
  ...
  _dataGetter: {
    ...
    getUpdatedDataMatrix(spIOManager, todayObj) {
      ...
      const newMaterialDataMatrix = this._getNewMaterialDataMatrix(thisYearNum, thisMonthNum);
      ...
      const finalIdsArray = this._getCombinedIdArray(oldMaterialDataMatrix, newMaterialDataMatrix);

      if (materialsCapacity < finalIdsArray.length) {
        throw new Error(`${changeMaterialsClass._LOGTAG}:教材数が限界値${materialsCapacity}を超えています。\n現在の教材数: ${finalIdsArray.length}`);
      }

      //「ID（prefixなし）」「科目名」「参考書名」「月間目標」「週ごとの計画時間(五列)」「週ごとの実績(五列)」「単位当たり処理量」の二次元配列。
      const finalDataMatrix = this._getFinalMatrix(finalIdsArray, oldMaterialDataMatrix, newMaterialDataMatrix);
    ...
    //NOTE: 非自明な行列操作が続いています。これはgetMonthlyDataが、01_minor_projects.gsに定義されているグローバル関数であり、SheetIOによるシート構成の影響吸収の恩恵を受けられていないためです。詳しくはgetMonthlyData関数のコメントをご覧ください。
      const baseMat = getMonthlyData(String(targetYearNum), String(targetMonthNum));
      const outputMat = [];
      for (let i = 0; i < baseMat.length; i++) {//データがないところは放置
    ...
    _getCombinedIdArray(oldMaterialDataMatrix, newMaterialDataMatrix) {
      ...
      newMaterialDataMatrix.forEach(row => {//基本的には、新教材の順を尊重するので優先的に、新計画の中のidを追加していく
        const id = String(row[colIdx.id]).trim();
        outputSet.add(id);
      });

      oldMaterialDataMatrix.forEach(row => {//実績があるのに新教材から消してしまった古い教材は最後尾に追加する。
     ...
     * @private
     */
    _getFinalMatrix(finalPureIdsArray, oldMaterialDataMatrix, newMaterialDataMatrix) {
      const colIdx = changeMaterialsClass._colToIndex;

      // IDから「参考書マスター」を検索して、「教科」と「月間目標」と「単位当たり処理量」を取得し、行列に代入
      //TODO:ここは、あたらしいstaticなapiに変えたい。
      const materialsMaster = MaterialsMasterLib.getMaterialsMaster(finalPureIdsArray, ["教科", "月間目標", "単位当たり処理量"]);
      ...
      const outputMatrix = Array.from({ length: finalPureIdsArray.length }, () => self._getEmptyRow());

      //新旧データを参照するために、indexでアクセスする必要があるので、教材idと対応する行のindexの対応mapを作る。
      //getSrcRowFromMapsが参照するので、宣言はここで。
      const oldIdMap = new Map(oldMaterialDataMatrix.map((row, idx) => [String(row[colIdx.id]).trim(), idx]));
      const newIdMap = new Map(newMaterialDataMatrix.map((row, idx) => [String(row[colIdx.id]).trim(), idx]));
      /**
      ...
      function getSrcRowFromMaps(id) {
        ...
        if (rowIdx !== undefined) {//oldにデータありならそれを採用
          srcRow = oldMaterialDataMatrix[rowIdx];
        } else {//oldにデータなし
          rowIdx = newIdMap.get(id);
          srcRow = (rowIdx !== undefined) ? newMaterialDataMatrix[rowIdx] : undefined;
        }
        return srcRow;
      }
      ...
      for (let i = 0; i < finalPureIdsArray.length; i++) {
        ...
        if (setMaterialName && !setSubject) {//科目名が空白なら参考書名から推測して補完
          outputMatrix[i][colIdx.subject] = MaterialsMasterLib.guessSubjectsByName(setMaterialName);
        }
  ...
  _dataSetter: {
    ...
    setData(spIOManager, todayObj, updatedDataMatrix) {
      ...
      const subjectsMatrix = updatedDataMatrix.map(row => [row[colIdx.subject]]);       // 科目名
      const referenceBookNamesMatrix = updatedDataMatrix.map(row => [row[colIdx.name]]); // 参考書名
      const monthlyGoalsMatrix = updatedDataMatrix.map(row => [row[colIdx.monthGoal]]);   // 月間目標

      const numPerUnitMatrix = updatedDataMatrix.map(row => [row[colIdx.numPerUnit]]);   // 単位当たり処理量
      ...
      this._updateMonthDataArchive(
        spIOManager, todayObj,
        pureIdsMatrix, subjectsMatrix, referenceBookNamesMatrix,
        monthlyGoalsMatrix, totalStudyTimeMatrix, numPerUnitMatrix
      );
