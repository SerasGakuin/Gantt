 ...
 *
 * - 00_FunctionConfigs.gsの、FunctionConfigsという配列に、同ファイルにあるコメントの説明通りにfuncXの設定を追加すればokです。
 ...
 *
 * - 最初に、00_FunctionConfigs.gsの、ContextFunctionConfigsという配列に、同ファイルにあるコメントの説明通りにfuncXを追加
 *
 ...
 *
 * ※functionIdを関数名とする仕組みに、現状なっていますが、安全のためにはidを明示的に設定したほうが良いかもしれません。その場合は00_FunctionsRunnerService.gsのidからfunctionへのマップ生成クラスと、htmlでのidの取得方法を両方ともconfig内のfunctionのnameを得るやり方から、idを得るやり方にして、00_FunctionConfigs.gsのコンフィグすべてにidプロパティを追加する必要があります。
 *
 *
 * ### 仕組み
 *
 ...
 *
 * ※戻り値を受けないとhtmlのscriptからの呼び出し時の連携に支障が出るので、本来ならば
 * function abstractFunction(functionId) { return Gantt.abstractFunction(functionId); }
 * のように戻り値を返すようにすべきでした。（戻り値を返さない場合でもこれでエラーにはなりません。）
 * しかし、現状returnしていないので一部機能に制限があります。詳しくは00_sidebar_interface.htmlを
 *
 *
 * 2026-03-14 追記：
 * 「年間計画レビュー」ドキュメントのGASの共通化に伴い、「ガントチャート」と同様に同ドキュメントについても個別の生徒の呼び出し元に共通インターフェースを実装しました。
 * この際、各呼び出し元のabstractFunctionの引数名は「request」としました。
 * 上記の通り、実際の動作には変化がありませんが、語弊の無い命名を採用しました。
 *
 * 「年間計画レビュー」のライブラリと呼び出し元は関数実行の結果をhtml側のscriptにreturnするようにしました。
 * これによってhtmlのGAS関数の呼び出しで、GAS関数実行結果を受け取れるようになっています。
 * このライブラリ、ガントチャートテンプレートの方は残念ながら現在できません。
 *
 *
 *
 * ### 呼び出しの挙動
 ...
 *
 * 8. FunctionRunnerServiceは、00_FunctionConfigs.gsに定義された設定から自動でfunctionIdに関数を対応させるマップを生成し、そのマップに渡されたfunctionIdに対応する関数があるかどうかチェック。
 ...
 *
 * 00_FunctionConfigs.gsの設定を参照して、わたされたfunctionIdとcontextを解釈して適切な関数に処理をわたすクラスを定義している。
 * 基本的に、関数追加時に改修の必要はない。
 *
 *
 ...
 *
 * 00_FunctionConfigs.gsの設定を参照して、htmlに「引数のない関数」の呼び出し用のボタンを注入しつつ、サイドバーメニューを表示するクラスを定義するファイル。
 * 基本的に、関数追加時に改修の必要はない。
 *
 *
