 ...
 * - FunctionRunnerService
 * 
 * api用のクラス。インスタンス化して、コールバックを設定してgenerateメソッドにfunctionIdとcontextを渡すことで実行できる。contextがnullでよいか、などは00_FunctionsConfigs.gsの定義に依存し、関数によって、つまり関数と一対一対応するfunctionIdごとに違う。
 * また、これだけがこのファイル外の00_番台からアクセスされうるapi用のクラス。他は補助である。
 * 
 * 
 * - FunctionRunnerService_IdToFuncMapFactory
 * 
 * 00_FunctionsConfigs.gsの定義に依存してfunctionId(string) to function(function) のマップオブジェクトを生成するクラス。
 * マップの生成時には、00_FunctionsConfigs.gsで、FunctionsConfigsやContextFunctionsConfigsが正しく設定されていることを仮定しておこなう。
 * 
 * 
 ...
 * 
 * 1. ContextFunctionsConfigsで必要なcontextの中身を指定して、それを満たさないものが来たらエラーをだすように仕様変更
...
class FunctionRunnerService {
  ...
  setOnStartedListener(func) { this._validateCallBackFunc(func); this._onStartedListener = func; } //関数実行開始時
  /** @param {function} func - 引数なしの関数 */
  setOnSucceededListener(func) { this._validateCallBackFunc(func); this._onSucceededListener = func; } //関数実効成功時
  /** @param {function} func - 引数なしの関数 */
  setOnFailedListener(func) { this._validateCallBackFunc(func); this._onFailedListener = func; } //関数実行失敗時
  /** @param {function} func - 引数なしの関数 */
  setOnFinishedListener(func) { this._validateCallBackFunc(func); this._onFinishedListener = func; } //関数実行終了時（成功/失敗に関係なし）
  ...
  run(functionId, context = {}) {
    ...
    try {
      if (!lock.tryLock(30000)) throw new Error('FunctionRunnerService:他の処理が実行中です。時間を置いて再度お試しください。');
      ...
      const targetFn = FunctionRunnerService_IdToFuncMapReader.findFunctionFrom(functionMap, functionId);
  ...
  _safeExecuteCallback(func) {
    try { func(); } catch (e) { console.error("Callback Error:", e); }
  }
   ...
   * @private
   */
  _runFunction(targetFn, context) {//関数実行
    FunctionRunnerService._runCount = (FunctionRunnerService._runCount || 0) + 1;//実行回数の初期化またはインクリメント
    if (FunctionRunnerService._runCount > 100) {
      throw new Error(`FunctionRunnerServiceが今回の実行時間中に100回以上呼ばれています！再帰ループまたは過剰なループによる無限実行が疑われます！FunctionRunnerServiceによって実行される関数の中でFunctionRunnerService.runを実行しないでください！`);
    }
    if (!!targetFn && typeof targetFn === 'function') targetFn(context);//実行
  }

  /** @returns {number} */
  static get _runCount(){return FunctionRunnerService.__runCount}
  /** @param {number} countNum */
  static set _runCount(countNum){FunctionRunnerService.__runCount = countNum}
   ...
   * @private
   */
  _validateCallBackFunc(func) { if (typeof func !== 'function' || func.length !== 0) throw new Error('引数のない関数を渡してください。') }

  // コールバックの関数たちのgetter
  get onStartedListener() { return this._onStartedListener || (() => { }) }
  get onFinishedListener() { return this._onFinishedListener || (() => { }) }
  get onSucceededListener() { return this._onSucceededListener || (() => { }) }
  get onFailedListener() { return this._onFailedListener || (() => { }) }

  set onStartedListener(func) { this._validateCallBackFunc(func); this._onStartedListener = func; } //関数実行開始時
  set onSucceededListener(func) { this._validateCallBackFunc(func); this._onSucceededListener = func; } //関数実効成功時
  set onFailedListener(func) { this._validateCallBackFunc(func); this._onFailedListener = func; } //関数実行失敗時
  set onFinishedListener(func) { this._validateCallBackFunc(func); this._onFinishedListener = func; } //関数実行終了時（成功/失敗に関係なし）
}
...
class FunctionRunnerService_IdToFuncMapFactory {
  ...
  static _generateFunctionsMap() {
    ...
    ContextFunctionsConfigs.forEach(obj => {
      const func = obj.func;
      const functionId = obj.id;
...
function test_FunctionRunnerService_IdToFuncMapReader() {
  ...
  const fn = FunctionRunnerService_IdToFuncMapReader.findFunctionFrom(map, functionId);
...
class FunctionRunnerService_IdToFuncMapReader {
  ...
  static findFunctionFrom(mapObj, functionId) {
    try {
      if (typeof mapObj !== 'object') throw new Error(`findFunctionFrom(mapObj, functionId)に渡されたmapObjの型が不正です。\n渡されたもの:${mapObj}, 型:${typeof mapObj}`);
      if (typeof functionId !== 'string') throw new Error(`findFunctionFrom(mapObj, functionId)に渡されたfunctionIdの型が不正です。\n渡されたもの:${functionId}, 型:${typeof functionId}`);

      const fn = mapObj[functionId];
      if (typeof fn !== 'function') this._pushDetailLogOnNotFoundFunc(functionId, mapObj);//idが非対応の時
  ...
  static _pushDetailLogOnNotFoundFunc(functionId, mapObj) {
    ...
    throw new Error(invalidIdView + closestCorrectIdsView + similarGlobalFuncsView);
  }
  ...
  static _getClosestIdsList(target, functionMap) {
    try {
      ...
      //idの距離を配列に格納
      const idsDistancesArr = correctIdsArr.map(v => this._getLevenshteinDistance(v, targetStr));

      //最短距離を記録
      const minDist = Math.min(...idsDistancesArr);

      //遠すぎるならnull
      const dynamicThreshold = Math.min(Math.max(3, Math.floor(targetStr.length / 2)), 7);
      if (minDist > dynamicThreshold) return null;

      //最短のidたち
      const closestIds = correctIdsArr.filter((_, i) => idsDistancesArr[i] === minDist);
      if (closestIds.length === 0) return null;

      return `[ ${closestIds.join(', ')} ]`;
    } catch (e) {//この機能はあくまで開発補助なので、ここで問題が起きた場合は記録だけ
      console.error(`渡されたidに近い正しいidをサジェストする関数「_getSimilarNameGlobaleFuncs」でエラーが発生しました。\n${e}`);
    }
  }
  ...
  static _getSimilarNameGlobaleFuncs(target) {
    try {
      ...
      // GASのグローバル関数を抽出
      const globalFuncsNamesArr = Object.keys(globalObj).filter(key => typeof globalObj[key] === 'function');

      if (globalFuncsNamesArr.length === 0) return null;

      const distances = globalFuncsNamesArr.map(v => this._getLevenshteinDistance(v, targetStr));
      ...
      const closestNames = globalFuncsNamesArr.filter((_, i) => distances[i] === minDist);
      return `[ ${closestNames.join(', ')} ]`;
    } catch (e) {//この機能はあくまで開発補助なので、ここで問題が起きた場合は記録だけ
      console.error(`グローバルに渡されたidと名前の似た関数があった場合、それの存在を提示する関数「_getSimilarNameGlobaleFuncs」でエラーが発生しました。\n${e}`);
    }
  }
