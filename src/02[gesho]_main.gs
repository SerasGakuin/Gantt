...
class Gesho {
   ...
   * @public
   */
  static start(isGanttChartUsed, dependencies) {
    //「ガントチャート」を使用しないならfalse、使用するならtrue

    if (isGanttChartUsed)
      Logger.log(
        `ガントチャートの「ある」ブックに対する月末月初処理を実行します`,
      );
    else
      Logger.log(
        `ガントチャートの「ない」ブックに対する月末月初処理を実行します`,
      );
    ...
    const yearAfterGeshoNum =
      monthBeforeGeshoNum === 12 ? yearBeforeGeshoNum + 1 : yearBeforeGeshoNum;
    const monthAfterGeshoNum =
      monthBeforeGeshoNum === 12 ? 1 : monthBeforeGeshoNum + 1;

    const theDateAfterGesho = new Date(
      yearAfterGeshoNum,
      monthAfterGeshoNum - 1,
      15,
    ); //年月が正しければいいので日は適当にちょっとずれても年月が変化ないように真ん中あたり

    Logger.log(
      `月末月初実行前の年月は、年:${yearBeforeGeshoNum}, 月:${monthBeforeGeshoNum}です。`,
    );
    Logger.log(
      `月末月初実行後の年月は、年:${yearAfterGeshoNum}, 月:${monthAfterGeshoNum}です。`,
    );

    //======================= 処理本体開始 ==========================

    //週間管理の古いデータを月間管理と月初に格納して、週間管理からは削除する。
    this._archiveWeeklyData(
      spIOManager,
      yearBeforeGeshoNum,
      monthBeforeGeshoNum,
    );
    ...
    this._archiveThisMonthPlanData(
      spIOManager,
      yearBeforeGeshoNum,
      monthBeforeGeshoNum,
    );
    ...
    if (isGanttChartUsed) {
      this._setNextMonthMaterialsWithGantt(
        spIOManager,
        yearAfterGeshoNum,
        monthAfterGeshoNum,
        dependencies,
      ...
      this._setNextMonthMaterialsWithoutGantt(
        spIOManager,
        yearAfterGeshoNum,
        monthAfterGeshoNum,
        dependencies,
    ...
    //日付関連の情報を更新。
    //IMPORTANT: ここまでに「今月のデータ」(アクティブなデータ)を取得して格納する場面があり、日付の設定に「今月のデータ」は追従するので、日付のセットはgeshoの最後に行う必要がある。
    ...
    //なにか一定期間のみ行いたい更新があればこのクラス（02_updateOnGesho.gsのクラス）に定義しておけばここで実行される。更新がない時でもこの呼び出し一行は置いておいて下さい。
  ...
  static _validateGeshoState(isGanttChartUsed, spIOManager) {
    ...
    if (notFoundSheetNames.length !== 0)
      throw new Error(
        `以下の必須シートが見つかりません: ${notFoundSheetNames.join(", ")}`,
      );
  }

  //週間管理の古いデータを月間管理と月初に格納して、週間管理からは削除する。
  static _archiveWeeklyData(
    spIOManager,
    yearBeforeGeshoNum,
    monthBeforeGeshoNum,
  ) {
    ...
    spIOManager.monthlyFirstSheet
      .getRange(4, colBJ, achievements.length, achievements[0].length)
      .setValues(achievements);

    // 週間管理の学習実績を月間管理に格納する
    spIOManager.archiveMaterialsAchievements(
      yearBeforeGeshoNum,
      monthBeforeGeshoNum,
      achievements,
    );
  ...
  static _archiveThisMonthPlanData(
    spIOManager,
    yearBeforeGeshoNum,
    monthBeforeGeshoNum,
  ) {
    Logger.log("今月プランの古いデータを今月実績や月初にコピーします。");
    const prefix = spIOManager.getYearMonthPrefix(
      yearBeforeGeshoNum,
      monthBeforeGeshoNum,
    );

    const lastIDsMatrix = spIOManager
      .getActiveMaterialsIds()
      .map((row) => [prefix + row[0]]);
    ...
    spIOManager.thisMonthAchievementSheet
      .getRange(4, 1, lastIDsMatrix.length, 1)
      .setValues(lastIDsMatrix); //先月の教材を今月プランから今月実績に貼りつけ。A4起点。
    spIOManager.thisMonthAchievementSheet
      .getRange(4, 4, lastTimeMatrix.length, 5)
      .setValues(lastTimeMatrix); //今月実績に勉強時間貼りつけ。D4起点。
  }
  ...
  static _setNextMonthMaterialsWithGantt(
    spIOManager,
    yearAfterGeshoNum,
    monthAfterGeshoNum,
    dependencies,
    ...
    const materialsMatrix = dependencies.getMonthlyData(
      yearAfterGeshoNum,
      monthAfterGeshoNum,
    );
    if (!materialsMatrix || materialsMatrix.length === 0) {
      throw new Error(
        "「ガントチャート」の来月のデータ（来月に使用する参考書のデータ、それぞれの週ごとの勉強時間、累計勉強時間）が空です。",
      );
    }
    ...
    const studyTimes = materialsMatrix.map((row) => [
      row[2],
      row[3],
      row[4],
      row[5],
    ...
    const totalStudyTimes = studyTimes.map((row) => {
      //合計勉強時間
    ...
    const materialsMaster = dependencies.getMaterialsMaster(
      refBookIdsFlatArray,
      ["教科", "月間目標", "単位当たり処理量"],
    );
    ...
    if (refBookIdsFlatArray.length <= materialsCapacity) {
      ...
      spIOManager.registerMaterialsMetadataToArchive(
        yearAfterGeshoNum,
        monthAfterGeshoNum, //年月prefixのついていない状態
        referenceBookIds,
        subjects,
        referenceBookNames,
        totalStudyTimes,
      ...
      console.log(
        "教材数が限界を超えています。来月のデータの自動代入はできないので、先月データのクリアのみを行います。\n",
      );
      spIOManager.thisMonthPlanSheet
        .getRange(4, 4)
        .setValue(
          "行数が今月プランの限界を超えていたのでデータのアーカイブのみを行いました。",
        );
    }
  }
  ...
  //先月のものをそのまま移行するだけ。
  static _setNextMonthMaterialsWithoutGantt(
    spIOManager,
    yearAfterGeshoNum,
    monthAfterGeshoNum,
    dependencies,
