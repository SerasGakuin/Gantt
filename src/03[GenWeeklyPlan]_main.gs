 ...
 */
function week1Plan() {
  genericGenWeeklyPlanProdFunc(1);
}
function week2Plan() {
  genericGenWeeklyPlanProdFunc(2);
}
function week3Plan() {
  genericGenWeeklyPlanProdFunc(3);
}
function week4Plan() {
  genericGenWeeklyPlanProdFunc(4);
}
function week5Plan() {
  genericGenWeeklyPlanProdFunc(5);
}

/**
 * 本番実行用の関数。
 * @param {number} weekNum - 計画を生成したい週の番号
 * @param {string} spId - 計画を生成したいスピードプランナーのid。2026-03-30現在はスプレッドシートなのでファイルid。
 * @private
 */
...
class GenWeeklyPlan {
  ...
  static start(weekNum, dependencies) {
    ...
    const isRunAllowed = GenWeeklyPlan_processGuard.isRunAllowed(existingAchievements, weekIndex);
    if (!isRunAllowed) return;

    //現在の教材の関連データをcontextオブジェクトに詰め込んでcontextたちの配列として取得。
    const materialsContextsArr = this._collectInfoAsContexts(spIOManager, dependencies, weekNum);

    // 教材ごとに処理する。
    for (let i = 0; i < materialsContextsArr.length; i++) {
      const context = materialsContextsArr[i];
      if(!context.materialInfo){//デバッグ用に参考書マスターに情報がない場合は
        console.warn(`参考書マスターに情報なし。\nID：${String(context.materialId)}\n最新の学習実績：${String(context.lastAchievement)}`);
      }
  ...
  static _collectInfoAsContexts(spIOManager, dependencies, weekNum) {
    ...
    function sanitizeLastAchievement(value) {
      ...
      output = output.replace(/[０-９]/g, function (s) {//全角の数字を半角に
    ...
    const lastAchievements = spIOManager.getActiveMaterialsAchievementsUpTo(weekNum).map(v => sanitizeLastAchievement(v[0]));
    ...
    for (let i = 0; i < materialsIds.length; i++) {
      const context = GenWeeklyPlan_contextFactory.create(
        dependencies,
        materialsIds[i], materialsData[materialsIds[i]],
        lastAchievements[i], valuesToAddArr[i]
      );
