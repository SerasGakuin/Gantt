...
/**
 * @deprecated  
 * 01_MatrixUtils.gsのMatrixUtils.transposeに移行することをおすすめします。後方互換性のために残しています。
 * @type {(a: any) => any}
 */
...
function getMonthlyData(year, month) {
  ...
  function collectYearMonthCols(dateRow) {
    ...
    for (let i = DATE_START_COL_INDEX; i < dateRow.length; i++) {
      if (isThisYearMonth(dateRow[i])) {//指定年月を発見
        cols.push(i);
      } else if (cols.length !== 0) {//連続範囲である前提なので、すでに指定年月の列を見つけた後に違う年月に出会ったら終了
        break;
      }
    }
    return cols;
  ...
  const yearMonthStudyTimeEndCol = studyTimeIndexArray[studyTimeIndexArray.length - 1];//終了行
  ...
  const filteredRows = ganttMatrix.filter(row => {
    ...
    const hasMaterialName = row[MATERIALS_NAMES_COL_IDX]?.toString().trim() !== "";
    const isMaterialRow = hasMaterialId && hasMaterialName;
    if (!isMaterialRow) return false;//参考書の行でないなら除外
    //勉強時間が0でないか判定
    const studyTimes = row.slice(yearMonthStudyTimeStartCol, yearMonthStudyTimeEndCol + 1);
    const totalStudyTime = studyTimes.reduce((sum, value) => sum + (Number(value) || 0), 0);
...
function displayMonthlyDataFromGantt(year, month) {
  ...
  if (Number.isNaN(yearNum) || Number.isNaN(monthNum)) {
    throw new Error(`正常な年月の数字を渡してください。\nyear:${year}, month:${month}`);
  }
  ...
  if (monthData.length == 0) {
    ...
    Logger.log(`警告: 指定された年月のデータが存在しませんでした。空データが月間実績に代入されます。\nyear:${year}, month:${month}\n${stackTrace}`);
    return;
  }
