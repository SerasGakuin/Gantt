...
class GenWeeklyPlan_contextFactory {
   ...
   */
  static create(dependencies, materialId, materialInfo, lastAchievement, valueToAdd) {
    const context = {
      dependencies,
  ...
  static _isCountedOnlyByChapter(materialInfo) {
    ...
    //章の数
    const chapterCount = Math.min(chapterStartsNums.length, chapterEndsNums.length);
    ...
    for (let i = 0; i < chapterCount; i++) {
      ...
      const isLackOfStartOrEnd = s === undefined || s === '' || e === undefined || e === '';
