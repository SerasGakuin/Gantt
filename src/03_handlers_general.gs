...
const GenWeeklyPlan_handlers_general = Object.freeze({
  ...
  noStudyHandler: {
    name: "noStudyHandler",
    run(context) {
      if (!context.valueToAdd)
        return GenWeeklyPlan_handlers_defaultTexts.onCanceled;
      else return null;
    },
  },
  ...
  noMaterialInfoHandler: {
    ...
    run(context) {
      if (!context.hasValidMetadata)
        return GenWeeklyPlan_handlers_defaultTexts.onNoMasterData;
      else return null;
    },
  },
  ...
  emptyHandler: {
    ...
    run(context) {
      if (!context.lastAchievement)
        return GenWeeklyPlan_handlers_defaultTexts.onConsultationNeeded;
      else return null;
    },
  },
  ...
  pastExamsHandler: {
    ...
    run(context) {
      ...
      const isPastBook =
        ...
        (materialName.includes("過去問") ||
          materialName.includes("共通テスト"));
  ...
  probNumberRangeHandler: {
    ...
    run(context) {
      //まず教材の性質で判定
      const isNotTargetMaterial =
        !context.hasValidMetadata ||
        context.isCountedOnlyByChapter ||
        context.isCountingRepeating;
      ...
      if (willFinish)
        return baseText + GenWeeklyPlan_handlers_defaultTexts.onFinished;
  ...
  probSingleNumberHandler: {
    ...
    run(context) {
      ...
      const divided = this._getComponents(
      ...
      const newEndNumber = Math.min(
        finalNum,
        currentNumber + context.valueToAdd,
      );
  ...
  chapProbRangeHandler: {
    ...
    run(context) {
      ...
      if (!chapterNames || !endNums || !Array.isArray(chapterNames))
        return null; //この方式はキャンセル
      ...
      // すでに終了しているか
      if (chap2Index == lastChapterIndex && problem2 == lastProblemNum) {
        return GenWeeklyPlan_handlers_defaultTexts.onFinished;
      }
      ...
      // === 次の終了位置を計算 ===
      const [endChapterIndex, endProblemNum] = this._getWhereProblemIsByIndex(
        startChapterIndex,
        startProblemNum,
        context.valueToAdd,
        endNums,
      ...
      const willFinish =
        endChapterIndex === lastChapterIndex &&
        endProblemNum === lastProblemNum;
      ...
      const isChapterOutOfRange =
        startChapterLabel === undefined || endChapterLabel === undefined;
      if (isChapterOutOfRange) return null; //万が一、章が範囲外なら

      const baseResult = this._constructBaseResult(
        firstText,
        startChapterLabel,
        commonUnit,
        startProblemNum,
    ...
    */
    _getComponents(inputString) {
      ...
      for (let i = 4; i < parts.length - 2; i++) {
        //4番目からparts.length-2番目の間にしか~はあり得ない。
        if (parts[i].includes("~")) {
          tildePos = i;
          break;
        }
      ...
      return [
        ...
        parts[tildePos],
        finalText,
        Number(parts[tildePos - 3]),
        Number(parts[tildePos - 1]),
        Number(parts[tildePos + 1]),
        Number(parts[tildePos + 3]),
      ];
    },
    ...
    _getWhereProblemIsByIndex(
      ...
      // 最初の章で残り何問あるか
      let firstRemain =
        Number(endNums[startChapterIndex]) - startProblemNum + 1;
    ...
    _constructBaseResult(
      ...
      const isSingleProb =
        startChapterLabel === endChapterLabel &&
        startProblemNum === endProblemNum;
