...
class GenWeeklyPlan_handlers_generator {
  ...
  static generate(context) {
    ...
    const willTellWhatHandler =
      GenWeeklyPlan_handlers_flags.pushLogAboutWhatHandlerGenerated;

    for (const handler of handlers) {
      //ハンドラーを一個ずつ実行して、結果がnullでなくなったら終了。
  ...
  static _validateHandlers(targetHandlers) {
    ...
    function checkName(itemName) {
      //ハンドラーのnameプロパティのチェック用関数
      if (itemName === undefined) {
        errors.push(
          `nameプロパティが未定義なハンドラーが検出されました。:${JSON.stringify(item)}`,
        );
        return;
      }
      if (typeof itemName !== "string") {
        errors.push(
          `nameプロパティがstringでないハンドラーが検出されました。:${JSON.stringify(item)}`,
        );
        return;
      }
      if (seenNamesSet.has(itemName)) {
        errors.push(
          `nameプロパティが他と重複したハンドラーが検出されました。ハンドラー複製時の名前変更ミスなどが疑われます:${JSON.stringify(item)}`,
        );
      }
      seenNamesSet.add(itemName);
    }

    function checkRunFunc(runFunc) {
      //ハンドラーのrunというfunctionのチェック用関数
      if (runFunc === undefined) {
        errors.push(
          `runFuncプロパティが未定義なハンドラーが検出されました。:${JSON.stringify(item)}`,
        );
        return;
      }
      if (typeof runFunc !== "function") {
        errors.push(
          `runプロパティがfunctionでないハンドラーが検出されました。:${JSON.stringify(item)}`,
        );
        return;
      }
      if (runFunc.length !== 1) {
        //これで引数の数を検出できる
        errors.push(
          `runプロパティのlengthが1でない(つまり取る引数がcontextの一つのみでない)ハンドラーが検出されました。:${JSON.stringify(item)}`,
        );
      }
    }
    ...
    targetHandlers.forEach((item) => {
      if (!item || typeof item !== "object") {
        errors.push(
          `オブジェクトでないハンドラーが検出されました。:${JSON.stringify(item)}`,
        );
        return;
      }
      if (seenHandlersSet.has(item)) {
        errors.push(
          `複数回_getHandlersArray()内に登録されているハンドラーが検出されました。:${JSON.stringify(item)}`,
        );
      }
