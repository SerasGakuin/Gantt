...
function test_GenWeeklyPlan_prod_like_generate() {
  ...
  const mockInput = {
    materialId: "gMB011", //参考書id
    lastAchievement: "数B：例題51～55", //最新の学習実績
    valueToAdd: 5, //目安処理量
  };
  ...
  */
  ...
  //参考書マスターから情報を検索。
  const materialInfo = dependencies.getMaterialsInfoByIds([
    mockInput.materialId,
  ])[mockInput.materialId];
  ...
  const context = GenWeeklyPlan_contextFactory.create(
    ...
    mockInput.materialId,
    materialInfo,
    mockInput.lastAchievement,
    mockInput.valueToAdd,
  );
...
function test_GenWeeklyPlan_generate_withAI_withRefBookInfo() {
  ...
  //実際に生成。
  const result =
    GenWeeklyPlan_handlers_withAI.askingAIWithBookInfoHandler.run(context);
...
function test_GenWeeklyPlan_generate_withAI_withoutRefBookInfo() {
  ...
  //実際に生成。
  const result =
    GenWeeklyPlan_handlers_withAI.askingAIWithoutBookInfoHandler.run(context);
