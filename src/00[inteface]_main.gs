...
function abstractFunction(idObj) {
  ...
  if (!idObj || typeof idObj !== 'object') {
    throw new Error('abstractFunctionの引数はオブジェクトである必要があります。\n詳しい実装については00[Document].gsをご覧ください。');
  }
  ...
  if (!id || typeof id !== 'string') {
    throw new Error(`渡されたオブジェクトに適切なid(string)が入っていません。:\n${JSON.stringify(idObj)}`);
  }

  if (context !== undefined && typeof context !== 'object') {
    throw new Error(`渡されたオブジェクトに不適切なcontext{object}が入っていました。contextはオブジェクトにするか、そもそも入れないかのいずれかにしてください。:\n${JSON.stringify(idObj)}`);
  }
...
function onOpenAction() {
  try {
    ...
    ToastNotificationService.send("メニューの読み込みに失敗しました: " + message);
    // 開発者はログ（コンソール）で詳細を確認
